...
from core.usecase.chat import ChatUsecase


async def generate_sse_stream(query_request: QueryRequest, chat_type: str):
    try:
...
async def handle_sse_stream(
    ...
    try:
        ...
        response = await generate_sse_stream(query_request, chat_type=chat_type.value)
        ...
        print(f"ERROR in SSE endpoint [{chat_type.name}]:", traceback.format_exc())
