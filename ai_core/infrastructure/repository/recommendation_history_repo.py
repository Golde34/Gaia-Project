...
class RecommendationHistoryRepository:
    ...
    async def get_by_user_and_context(self, user_id: str, context_id: str) -> Optional[RecommendationHistory]:
        query = (
            "SELECT id, user_id, context_id, fingerprint, recommendation, created_at "
            "FROM recommendation_history WHERE user_id=$1 AND context_id=$2 ORDER BY created_at DESC"
        )
        pool = await postgres_db.connect()
        async with pool.acquire() as conn:
            rows = await conn.fetch(query, user_id, context_id)
            return RecommendationHistory(**dict(rows[0])) if rows else None

    async def save(self, record: RecommendationHistory) -> int:
        existing = await self.get_by_user_and_context(record.user_id, record.context_id)
        if existing:
            query = (
            ...
            query = (
                "INSERT INTO recommendation_history (id, user_id, context_id, fingerprint, recommendation, created_at) "
                "VALUES ($1, $2, $3, $4, $5, $6) RETURNING id"
            )
            pool = await postgres_db.connect()
            async with pool.acquire() as conn:
                return await conn.fetchval(query, record.id, record.user_id, record.context_id, record.fingerprint, record.recommendation, record.created_at)
    ...
    async def should_recommend(self, user_id: Optional[str], fingerprint: str, context_id: Optional[str] = "default") -> bool:
        if not user_id:
            # if no user id, always recommend
            return True
        rec = await self.get_by_user_and_context(user_id, context_id)
        if rec is None:
            return True
        return rec.fingerprint != fingerprint
    
    async def register(self, user_id: Optional[str], fingerprint: str, recommendation: str, context_id: Optional[str] = "default", record_id: Optional[str] = None, created_at=None) -> None:
