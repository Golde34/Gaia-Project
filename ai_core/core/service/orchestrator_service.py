...
class OrchestratorService:
    ...
    async def execute(self, query: QueryRequest, tasks: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        parallel_futures: List[asyncio.Task[Any]] = []
        if parallel_tasks:
            parallel_futures = [
        ...
        context_payload = self._compose_recommendation_context(query, results)
        fingerprint = self._compose_fingerprint(context_payload)
    ...
    def _compose_fingerprint(self, context_payload: str) -> str:
