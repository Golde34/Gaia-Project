...
from core.service.chat_service import reflection_chat_history 
...
class ChatUsecase:
    ...
    async def _update_chat_history(cls, query: QueryRequest, response: str):
        ...
        if lt_queue_length >= long_term_memory_max_length_config:
    ...
    async def _update_long_term_memory(cls, user_id: str, dialogue_id: str):
        ...
        await send_kafka_message(
            kafka_enum.KafkaTopic.UPDATE_LONG_TERM_MEMORY.value, payload
        )
