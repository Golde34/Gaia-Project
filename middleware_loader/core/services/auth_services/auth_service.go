...
func (s *AuthService) GetServiceJWT(ctx context.Context, serviceName string) (string, error) {
	serviceJWT, err := client.IAuthAdapter(&adapter.AuthAdapter{}).GetServiceJWT(serviceName)
	if err != nil {
		return "", err
	} else {
