...
from core.domain.request.memory_request import MemoryDto, MemorySelectionToolDto, MemoryRequest
...
async def recall_history_info(query: QueryRequest, default=True):
    ...
    last_bot_message = memory_dto.recent_history.split(
        "Bot:")[-1].strip() if "Bot:" in memory_dto.recent_history else ""

    return MemorySelectionToolDto(
        query=query.query,
...
async def kafka_update_long_term_memory(memory_request: MemoryRequest, recent_history: str) -> None:
    ...
    try:
        ...
        if long_term_memory.content and len(long_term_memory.content) > 0:
            ...
            query_embeddings = milvus_validation.validate_milvus_insert(
                embedding)
            ...
            print(
                f"Saved {len(long_term_memory.content)} long term memory items to Milvus DB")
