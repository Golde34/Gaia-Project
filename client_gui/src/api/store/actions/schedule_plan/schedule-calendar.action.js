...
export const deleteTaskAwaySchedule = (payload) => async (dispatch) => {
    dispatch({ type: DELETE_TASK_AWAY_SCHEDULE_REQUEST, payload });
    try {
        const { data } = await serverRequest(`/schedule-calendar/schedule-day/delete-task-away-schedule`, HttpMethods.POST, portName.middleware, payload);
